...
pr_branch="merge/${INPUT_UPSTREAM_BRANCH}-to-${INPUT_BRANCH}-${last_sha}"
pr_title="Chore Merge: ${INPUT_UPSTREAM_BRANCH} to ${INPUT_BRANCH}"
...
if [[ "${up_to_date}" -eq 0 ]]; then
  ...
  if [[ "${pr_exists}" -gt 0 ]]; then
    ...
    git_cmd hub pull-request -b "${INPUT_BRANCH}" -h "${pr_branch}" -l "${INPUT_PR_LABELS}" -a "${GITHUB_ACTOR}" -m "\"${pr_title}\""
  fi
else
